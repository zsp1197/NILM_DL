...
def cut_series(series: pd.Series, startTime: pd.Timestamp, endTime: pd.Timestamp, concat=False, fill_zero=True):
    begin, end = min(series.index), max(series.index)
    cutted = series[startTime:endTime]
    remaining = [series[begin:startTime], series[endTime:end]]

    if (concat):
        remaining = pd.concat(remaining)
    if (fill_zero and concat):
        remaining=pd.Series(index=pd.date_range(start=begin, end=end, freq='S'), data=0).add(remaining, fill_value=0)
    #
    # if (fill_zero and concat):
